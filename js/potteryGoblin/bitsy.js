...
function checkStart() {
	...
	var gameCanvas = document.getElementById("game");
	const compStyles = window.getComputedStyle(gameCanvas);

	if (compStyles.getPropertyValue("display") === "none") {
		var pregameDiv = document.getElementById("pre-game");
...
function update(dt) {
	...
	if (transition && transition.IsTransitionActive()) {
		...
		if (!isNarrating && !isEnding && gamePlaying) {
...
function updateInput() {
	if (dialogBuffer && dialogBuffer.IsActive()) {
		...
		if (!isAnyButtonHeld && isAnyButtonDown() || !gamePlaying) {
